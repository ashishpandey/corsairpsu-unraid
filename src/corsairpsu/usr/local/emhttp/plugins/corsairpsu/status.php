...
if (!isset($data))
    exit(json_encode([$stdout], JSON_THROW_ON_ERROR));
...
if ($settings["TYPE"] == "corsairmi") {
	...
	$output0 = substr((string) $data['output0 volts'], 0, strrpos((string) $data['output0 volts'], '.'));
	$output1 = substr((string) $data['output1 volts'], 0, strrpos((string) $data['output1 volts'], '.'));
	$output2 = substr((string) $data['output2 volts'], 0, strrpos((string) $data['output2 volts'], '.'));

	// Removes raw timestamp on uptime / total hours leaving just the Xd, Xh text
	preg_match('/\((.*?)\)/', (string) $data['uptime'], $uptime);
	preg_match('/\((.*?)\)/', (string) $data['powered'], $poweredon);

	// Adds the keys and values to an array, named appropiately
	$json = ['temp1' => $data['temp1'], 'temp2' => $data['temp2'], 'fan_rpm' => $data['fan rpm'], 'capacity' => $capacity, "{$output0}v_watts" => $data['output0 watts'], "{$output1}v_watts" => $data['output1 watts'], "{$output2}v_watts" => $data['output2 watts'], 'watts' => $data['total watts'], 'load' => $load, "{$output0}v_load" => $output0_load, "{$output1}v_load" => $output1_load, "{$output2}v_load" => $output2_load, 'vendor' => $data['vendor'], 'product' => $data['product'], 'uptime' => $uptime[1], 'uptime_raw' => substr((string) $data['uptime'], 0, strpos((string) $data['uptime'], ' ')), 'poweredon' => $poweredon[1], 'poweredon_raw' => substr((string) $data['powered'], 0, strpos((string) $data['powered'], ' ')), 'input_voltage' => round($input_voltage), 'input_power' => round($est_input_power), 'efficiency' => floatval($efficiency)];	
} else {
	$capacity = filter_var($data["PSU type"], FILTER_SANITIZE_NUMBER_INT);
	$input = floatval($data['Input power']);
	$load = round($input / $capacity * 100);

	$rail_5v = explode(",", (string) $data['5V Rail']);
	$rail_3v = explode(",", (string) $data['3.3V Rail']);
	...
	$json = ['temp1' => floatval($data["Temperature"]), 'temp2' => floatval($data["Temperature"]), 'fan_rpm' => floatval($data["Fan speed"]), 'capacity' => $capacity, "12v_watts" => $rail_12v_watts, "5v_watts" => $rail_5v_watts, "3v_watts" => $rail_3v_watts, 'watts' => $input, 'load' => $load, "12v_load" => $rail_12v_load, "5v_load" => $rail_5v_load, "3v_load" => $rail_3v_load, 'vendor' => "CORSAIR", 'product' => str_replace("\r","",(string) $data["PSU type"]), 'uptime' => "Not Supported", 'uptime_raw' => "Not Supported", 'poweredon' => "Not Supported", 'poweredon_raw' => "Not Supported", 'efficiency' => floatval($data['Efficiency'])];
}
